...
#FIXME: Add plug-in-version-only data for session and persistent source

EXPECTED_PROCEDURE_ATTRIBUTES = [
  {
...
class TestUpdateFrom331To34(unittest.TestCase):
  ...
  def test_data_in_session_source_only(self, *mocks):
  ...
  def _test_lengths_of_groups(self):
    ...
    self._test_length_and_names_of_settings(
      self.settings['special'], ORIG_SETTINGS['special'])
    self._test_length_and_names_of_settings(
      self.settings['main'], ORIG_SETTINGS['main'])
    self._test_length_and_names_of_settings(
      self.settings['gui'], ORIG_SETTINGS['gui'])
    self._test_length_and_names_of_settings(
      self.settings['gui/size'], ORIG_SETTINGS['gui/size'])
  
  def _test_main_settings(self):
    self.assertEqual(self.settings['main/plugin_version'].value, pg.config.PLUGIN_VERSION)
  ...
  def _test_selected_and_collapsed_settings(self):
    self.assertFalse(self.settings['main/selected_layers'].value)
  ...
  def _test_contents_of_actions(self, actions, expected_num_actions, expected_action_attributes):
  ...
  def _test_contents_of_procedures(self):
    self._test_contents_of_actions(
      self.settings['main/procedures'], 6, EXPECTED_PROCEDURE_ATTRIBUTES)
  
  def _test_contents_of_constraints(self):
    self._test_contents_of_actions(
      self.settings['main/constraints'], 4, EXPECTED_CONSTRAINT_ATTRIBUTES)
