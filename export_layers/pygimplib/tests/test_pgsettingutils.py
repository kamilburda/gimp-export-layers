...
class TestPdbParamCreator(unittest.TestCase):
  ...
  def test_create_params_invalid_argument(self):
    with self.assertRaises(TypeError):
      pgsettingutils.PdbParamCreator.create_params([self.file_ext_setting])
  
  def test_create_multiple_params(self):
    params = pgsettingutils.PdbParamCreator.create_params(self.file_ext_setting, self.settings)
  ...
  def test_create_params_with_unregistrable_setting(self):
    params = pgsettingutils.PdbParamCreator.create_params(self.unregistrable_setting)
    self.assertEqual(params, [])
  
  def test_list_param_values(self):
    param_values = pgsettingutils.PdbParamCreator.list_param_values([self.settings])
  ...
  def test_list_param_values_ignore_run_mode(self):
    param_values = pgsettingutils.PdbParamCreator.list_param_values(
